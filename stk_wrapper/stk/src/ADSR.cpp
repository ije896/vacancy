...
ADSR :: ADSR( void )
{
  ...
  attackRate_ = 0.001;
  decayRate_ = 0.001;
  numAttackSamples_tmp = 0;
  numDecaySamples_tmp = 0;
  numAttackSamples_tmp = 0;
  numDecaySamples_tmp = 0;
  releaseRate_ = 0.005;
  releaseTime_ = -1.0;
...
void ADSR :: sampleRateChanged( StkFloat newRate, StkFloat oldRate )
{
  if ( !ignoreSampleRateChange_ ) {
    ...
    // numAttackSamples_ = newRate * numAttackSamples_ / newRate;
    // numDecaySamples_ = newRate * numDecaySamples_ / newRate;
  }
}
...
void ADSR :: setInitialValue( StkFloat value)
{
  if ( value < 0.0 ) {
    oStream_ << "ADSR::setInitialValue: negative value not allowed!";
...
void ADSR :: setAttackTime( StkFloat time )
{
  ...
  numAttackSamples_ = time * Stk::sampleRate();
  numAttackSamples_tmp = time * Stk::sampleRate();
}
...
void ADSR :: setDecayTime( StkFloat time )
{
  ...
  numDecaySamples_ = time * Stk::sampleRate();
  numDecaySamples_tmp = time * Stk::sampleRate();
