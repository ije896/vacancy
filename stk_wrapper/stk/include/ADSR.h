...
class ADSR : public Generator
{
 public:
  ...
  //! Set the envelope's starting value
  void setInitialValue( StkFloat value );
 ...
 protected:
  ...
  int numAttackSamples_;
  int numDecaySamples_;
  int numAttackSamples_tmp;
  int numDecaySamples_tmp;
};
...
inline StkFloat ADSR :: tick( void )
{
  switch ( state_ ) {
    ...
    case ATTACK:{
    numAttackSamples_tmp--;
    if(value_<target_){
      value_ += attackRate_;
    }
    if ( value_ >= target_ ) {
        if(numAttackSamples_tmp>0){
            value_ = target_;
        }
        if(numAttackSamples_tmp<=0){
          // move on
          ...
          state_ = DECAY;
          numAttackSamples_tmp = numAttackSamples_;
        }
    }
    lastFrame_[0] = value_;
    break;
    ...
    if ( value_ > sustainLevel_ ) {
      ...
      if ( value_ <= sustainLevel_ ) {
        value_ = sustainLevel_;
          if(numDecaySamples_tmp<=0){
              state_ = RELEASE;
              numDecaySamples_tmp = numDecaySamples_;
          }
      }
    }
    else {
